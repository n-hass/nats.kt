...
class ClientHelloValidationTest {
	...
	fun `clientHello legacy_version is 0x0303`() =
		tlsTest {
			...
			assertEquals("0x0303", hello["LEGACY_VERSION"], "legacy_version must be 0x0303 per RFC 8446 s4.1.2")
		}
	...
	fun `clientHello random is 32 bytes`() =
		tlsTest {
			val hello = captureClientHello()
			assertEquals("32", hello["RANDOM_LENGTH"], "random must be 32 bytes per RFC 8446 s4.1.2")
		}
	...
	fun `clientHello offers TLS_AES_128_GCM_SHA256`() =
		tlsTest {
			...
			assertContains(suites, "0x1301", message = "Must offer TLS_AES_128_GCM_SHA256 per RFC 8446 s9.2")
		}
	...
	fun `clientHello compression_methods is only null`() =
		tlsTest {
			val hello = captureClientHello()
			assertEquals("0x00", hello["COMPRESSION_METHODS"], "compression_methods must be [0x00] per RFC 8446 s4.1.2")
		}
	...
	fun `clientHello includes key_share extension`() =
		tlsTest {
			...
			assertContains(extTypes, "0x0033", message = "key_share extension (0x0033) must be present per RFC 8446 s4.2.8")
		}
	...
	fun `clientHello includes signature_algorithms extension`() =
		tlsTest {
			val hello = captureClientHello()
			val extTypes = hello["EXT_TYPES_PRESENT"] ?: ""
			assertContains(extTypes, "0x000d", message = "signature_algorithms extension (0x000d) must be present per RFC 8446 s4.2.3")
		}
	...
	fun `clientHello includes supported_groups extension`() =
		tlsTest {
			val hello = captureClientHello()
			val extTypes = hello["EXT_TYPES_PRESENT"] ?: ""
			assertContains(extTypes, "0x000a", message = "supported_groups extension (0x000a) must be present per RFC 8446 s4.2.7")
		}
	...
	fun `clientHello key_share has at least one entry`() =
		tlsTest {
			...
			assertTrue(groups.isNotEmpty(), "key_share must contain at least one group per RFC 8446 s4.2.8")
		}
}
